/* icons.js — replaces <iconify-icon icon="lucide:NAME"> with inline SVG.
   Inline SVG survives screenshot capture, print-to-PDF and PPTX export,
   which shadow-DOM web components do not. Authoring stays the same:
   write <iconify-icon icon="lucide:heart"></iconify-icon> and this hydrates it. */
(function () {
  const L = {
    ...
    "heart-pulse": '<path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="M3.22 12H9.5l.5-1 2 4 .5-2 .5 1h6.27"/>',
    "car-front": '<path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><path d="M9 17h6"/><circle cx="17" cy="17" r="2"/>',
    ...
    "image-down": '<path d="M10.3 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v10.3"/><path d="m21 15-3.1-3.1a2 2 0 0 0-2.814.014L6 21"/><circle cx="9" cy="9" r="2"/><path d="M19 16v6"/><path d="m22 19-3 3-3-3"/>',
    ...
    "hand-heart": '<path d="M11 14h2a2 2 0 0 0 2-2 2 2 0 0 0-2-2H6.5a2 2 0 0 0-1.79 1.11L3 14"/><path d="m7 20 1.4-1.4a2 2 0 0 1 1.4-.6H13a2 2 0 0 0 2-2c0-1 .8-1.9 1.8-3l2.4-2.5a2.1 2.1 0 0 0-3-3L13 9.5"/><path d="M2 16h.01"/><path d="M22 18h.01"/>',
  ...
  function hydrate(root) {
    (root || document).querySelectorAll('iconify-icon').forEach(function (el) {
      const name = (el.getAttribute('icon') || '').replace('lucide:', '');
      const paths = L[name];
      if (!paths) return;
      el.insertAdjacentHTML('beforebegin',
        '<svg class="ic-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' + paths + '</svg>');
      el.remove();
    });
  }
